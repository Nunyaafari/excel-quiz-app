...
export default function ResultsPage() {
  ...
  const badgeSvg = buildBadgeSvg({
    percentage,
    performanceLabel: performance.label,
    scoreLabel: `${latestAttempt.correctAnswers}/${totalQuestions} correct`,
    detailLabel: profileLabel,
  ...
  const createBadgePngBlob = async (): Promise<Blob> => {
    const svgBlob = new Blob([badgeSvg], { type: 'image/svg+xml;charset=utf-8' })
    ...
    try {
      const image = await new Promise<HTMLImageElement>((resolve, reject) => {
        ...
        img.onerror = () => reject(new Error('badge-image-load-failed'))
        img.src = svgUrl
      })
      ...
      if (!context) {
        throw new Error('badge-canvas-context-missing')
      }
      ...
      const pngBlob = await new Promise<Blob>((resolve, reject) => {
        canvas.toBlob((blob) => {
          ...
          reject(new Error('badge-png-generation-failed'))
        }, 'image/png')
      })
          ...
          <section className="rounded-2xl border border-[#d9e3ef] bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-5 grid grid-cols-1 gap-6 xl:grid-cols-[0.95fr_1.05fr]">
              <div className="overflow-hidden rounded-[28px] bg-gradient-to-br from-[#0f2744] via-[#144d6a] to-[#1f6f6d] p-6 shadow-lg">
                <div className="rounded-[24px] border border-white/20 bg-white/10 p-5 backdrop-blur-sm">
                  ...
                  <div className="mt-6 flex flex-wrap items-end justify-between gap-4">
                    <div>
                      <p className={`text-6xl font-bold ${performance.scoreClass}`}>{percentage}%</p>
                      <p className="mt-2 text-lg font-semibold text-white">{performance.label}</p>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3 text-right shadow-sm">
                      <p className="text-xs uppercase tracking-wide text-[#5f7491]">Score</p>
                      <p className="mt-1 text-xl font-bold text-[#142842]">
                        {latestAttempt.correctAnswers}/{totalQuestions}
                      </p>
                    </div>
                  </div>
                  <div className="mt-6 space-y-2 text-sm text-[#d6ebff]">
                    <p>{profileLabel}</p>
                    <p>Completed {completedLabel}</p>
                    <p>Excel Mastery Quiz</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-[#dbe5f1] bg-[#f8fbff] p-4">
                ...
                <p className="mt-3 break-words rounded-lg border border-[#dbe5f1] bg-white p-4 text-sm leading-relaxed text-[#1e3757]">
                  {shareCaption}
                </p>
                <div className="mt-3 rounded-lg border border-[#dbe5f1] bg-white px-4 py-3 text-xs text-[#5a6f8a]">
                  Share URL:{' '}
                  <span className="break-all font-medium text-[#1e3757]">
                    {shareUrl}
                  </span>
            ...
            <section className="rounded-2xl border border-[#d9e3ef] bg-white p-5 shadow-sm md:p-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#5a6f8a]">
                    We only show the summary score on-screen at first. Enter your email to unlock the full breakdown,
                    training recommendations, question review, and save an email-ready copy of your report.
                  </p>
                </div>
